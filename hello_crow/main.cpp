...
string getView(response &res, const string &filename, context &x) {
...
int main(int argc, char* argv[]) {
    ...
    //[db][collections]
    auto collection = conn["crow"]["contacts"];
    ...
    CROW_ROUTE(app, "/contact/<string>/<string>")
        ([&collection](const request &req, response &res, string job, string email){
            auto doc = collection.find_one(make_document(kvp("job", job), kvp("email", email)));
    ...
    CROW_ROUTE(app, "/rest_test").methods(HTTPMethod::Post)
        ([](const request &req, response &res) {
